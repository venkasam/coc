...
AFRAME.registerComponent("markerhandler", {
  init: async function () {
    ...
    this.el.addEventListener("markerFound", () => {
      ...
      var barcodeval=this.el.getAttribute("value")
      elementsArray.push({element_name:elementname,barcode_value:barcodeval})
      compounds[barcodeval]["compounds"].map(item=>{
        var compound=document.querySelector(`#${item.compound_name}-${barcodeval}`)
        compound.setAttribute("visible",false)


      })
      var atom=document.querySelector(`#${elementname}-${barcodeval}`)
            ...
            if (compound !== undefined)
             { var barcodeValue = elementsArray[0].barcode_value;
               this.showCompound(compound, barcodeValue); } 
                ...
                else { messagtext.setAttribute("visible", false); }
        ...
        for(var i of elementsArray){
         ...
         { 
          var count = this.countOccurrences(elementsArray, el.element_name);
           if (count > 1) { compound += count + i.element_name; 
